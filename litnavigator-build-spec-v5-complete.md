# LitNavigator · Complete Implementation Spec (v5)

> **Overview of v5 changes vs. v4 (review polish; SQLite schema unchanged):**
> 1. §3 routing adds a terminal branch `concede`, eliminating the inner-loop infinite loop when "reteach is exhausted and no prerequisite is missing."
> 2. §2.3/§4.3 define an observation-count update function for `ConceptState.confidence`, so S5 calibration is actually grounded.
> 3. §4.2/§6 introduce "parallel quiz forms" so the T5 learning-gain measurement holds up.
> 4. §5.2/§5.3/§5.4 change induced `confidence` from "an LLM-blurted number" to a **transparent heuristic rule**.
> 5. §12 adds an "incremental thin UI" to each milestone, resolving the "recordable snapshot vs. UI deferred to D9" contradiction.
> 6. §12 adds a three-person parallel-track ownership table.
> 7. §7 mandates LangGraph's native `SqliteSaver`; §12 starts cost accounting at M1; a note on prerequisite-state initialization.
> Every change point is marked **[v5]**.
...
> **[v5] Don't let the cost dimension run naked**: scoring includes "cost." Token/cost accounting **should not be deferred to M4** — from M1, drop a lightweight token count into `decisions` / `tutor_turns` (see §12), so the deck can report "average tokens/cost per concept learned."
...
input
  ↓
init_or_load_state            ← [v5] here, build default learner_state for all target/prereq concepts (prevents router KeyError)
  ↓
...
│                ├── [v5] reteach exhausted & prereqs OK ───► EXIT → concede (honest exit)        │
...
|---|---|---|
| `init_or_load_state` | Create/load the session's NavState; **[v5] build a default ConceptState for all target concepts and their prereqs** | — |
...
| `grade` | Judge correct/wrong + identify misconception + BKT-lite mastery update + **[v5] update confidence** + write evidence | **Doesn't decide routing** |
...
| `concede` | **[v5] honest termination when reteach is exhausted and no prereq is missing**: flag the concept's confidence as low, write an S5 honest message, advance and exit | Don't pretend to have taught it; don't reteach infinitely |
...
class ConceptState(TypedDict):
    ...
    confidence: float                    # how sure the system is about that estimate (→ S5); [v5] updated by observation count, see §4.3
    n_observations: int                  # [v5] number of times this concept has been graded (input to confidence)
...
class NavState(TypedDict):
    ...
    decision: Literal["advance","reteach","diagnose","replan",
                      "refuse_jump","induce","concede","finish"] | None  # [v5] +concede
...
def tutor_router(state) -> str:
    ...
    # prereq-OK check ([v5] init already built default state for all concepts, so no KeyError here)
    ...
    # 4) [v5] prereqs all OK, but reteach exhausted and still not mastered → concede honestly (kills the loop)
    #    Old v4 returned "diagnose" here, and diagnose_gap finds no missing prereq → replan spins →
    #    back to the same concept → reteach already capped → diagnose again …… infinite loop.
    return "concede"
...
> **[v5] Parallel quiz forms (so the T5 learning gain holds up)**: for each demo-core concept, prepare **2–3 equal-difficulty parallel items** (same `evidence_chunk`, same `qtype`, same `difficulty`, only the wording/option order differs). Before and after teaching (pre/post), **draw a different parallel item each time** — this avoids the contamination of re-asking the same item (testing effect) while keeping difficulty comparable, so `post>pre` is a real gain and not just an easier question. Schema unchanged (reuses `quiz_items.difficulty`); only the data-prep step pairs the items.

### 4.3 grade — identify "which misconception," BKT mastery update, [v5] and confidence update
...
def bkt_update(p, correct, taught):
    ...
    # [v5] note: with taught=True, (1-post)*P_TRANSIT is added even on a wrong answer — this is the
    #   standard BKT term, not a bug. Verified that under these params a wrong answer still falls back
...
### 5.3 `mine_misconception` (misconception induction) + [v5] transparent confidence formula
...
**[v5] Transparent confidence tiers for induced items (shared by prereq edges and misconceptions)**:
```python
...
- **[v5] confidence is computed by the evidence rule, not model conjecture**: display `confidence_basis` (how many papers, what language strength, whether multi-paper corroborated).
...
| **[v5] Parallel quiz forms** | **2–3 equal-difficulty parallel items per demo-core concept (pre/post draw one each, see §4.2)** |
...
CREATE TABLE quiz_items (
    ...
    difficulty INTEGER,                  -- [v5] parallel forms group by same concept+qtype+difficulty
...
CREATE TABLE learner_state (
    session_id TEXT REFERENCES sessions(id),
    concept_id INTEGER REFERENCES concepts(id),
    mastery REAL, confidence REAL,       -- [v5] confidence written by §4.3 confidence_update
    n_observations INTEGER DEFAULT 0,    -- [v5] input to confidence
...
CREATE TABLE tutor_turns (
    ...
    cited_chunks TEXT,
    token_cost INTEGER,                  -- [v5] lightweight cost accounting (§1.3/§12)
...
CREATE TABLE decisions (
    ...
    token_cost INTEGER,                  -- [v5] lightweight cost per decision step
    created_at TIMESTAMP
);
...
CREATE TABLE induction_log (
    ...
    confidence_basis TEXT,               -- [v5] JSON: n_chunks/max_strength/multi_paper
    created_at TIMESTAMP
);
```
...
> **[v5] Note**: the newly added `n_observations / token_cost / confidence_basis` are all **column-level additions** — no primary-key changes, no changes to existing columns, backward-compatible.
...
| **[v5] pre/post gain via parallel forms** | **the testing effect requires avoiding re-testing the same item; equal-difficulty parallel forms make pre/post comparable** |
...
| S5 calibration | consensus/contested/open tiers; induced low-confidence admits "tentative, can be overruled"; **[v5] on concede, admit "this hasn't landed" + when confidence is low with few observations, say so plainly** |
...
| **T2b [v5] concede termination** | reteach exhausted with no missing prereq → `concede` (flag low confidence + S5 honesty + advance), **no infinite loop** | M2 |
...
| T5 in-session learning gain | within tutor_turns, post_check_score > pre_check_score; **[v5] pre/post drawn from same-concept equal-difficulty parallel items** | M2 |
| **T5b [v5] confidence calibration** | confidence rises monotonically with n_observations; on a single observation, UI/rationale state "low confidence" | M2 |
| T6 induction with evidence | every induced edge/misconception has ≥1 cited chunk + source='induced' + induction_log; **[v5] with confidence_basis** | M3 |
| T7 induction demoable | demo shows ≥1 corpus-induced element (or induced offline + evidence chain shown live) | M3 |
| T8 honest provenance | UI/rationale distinguishes curated vs induced, low confidence uses S5 tone; **[v5] show that confidence is rule-computed** | M3 |
...
### [v5] Three-person parallel tracks (from D1; milestones are cross-track convergence points)
...
- **Deliverable**: the LangGraph state machine runs end-to-end on rails (however dumb): input→planner→present concept→fixed quiz→score→advance; NavState lands in SQLite (**[v5] via `SqliteSaver`**); **init builds a default ConceptState for all concepts**.
...
- **[v5] Thin UI increment**: a minimal panel (left chat / right route+evidence), so "the route changed because of your quiz" is **recordable on the spot**, not saved up to D9.
- **[v5] Cost**: decisions/tutor_turns start logging token_cost.
...
- **[v5] Thin UI increment**: add the three-color concept graph + reteach trail (tried_strategies visible) + dual mastery/confidence display.
...
- **[v5] Thin UI increment**: curated vs induced visual distinction + openable evidence + `confidence_basis` display.
...
- **D9**: **[v5] the UI has grown incrementally with the milestones, so D9 is only "final integration + polish"** (no longer building the frontend from scratch under pressure) + record the demo of **the highest phase reached**.
...
**[v5] Counterfactual (concede, optional to show)**: deliberately construct a concept that "still fails after two explanations, with prereqs all OK" → demonstrate `concede` honestly exiting, showing honesty and no infinite loop.
...
## 15. Positioning statements (for the deck and README)
