...
| ⑥ | **GRADE.** Answer *"to correct errors in quantum information"* → **correct**; feedback *"correctly identifies the purpose"*; mastery 0.30 → 0.48. | **BKT-lite** mastery heuristic (cf. Corbett & Anderson 1995) — **never LLM self-judgement** | node `grade_kp.py` · `litnav/state.py` (`kp_bump`) |
...
| ✻ | **Orchestration (every step).** The outer agent picks which skill to run per state. | **ReAct** (Yao et al., ICLR 2023; arXiv:2210.03629) + **Plan-and-Solve** (Wang 2023) | `litnav/graph/builder.py` (LangGraph `StateGraph` + SqliteSaver) |
| ✻ | **Cost (every call).** One metered router; cheap tier by default, frontier only when needed; mastery/difficulty updates are rule-based (free); per-session budget cap. | model cascade — **FrugalGPT** · **RouteLLM** (ICLR 2025) | `litnav/llm/{router,registry,result_cache}.py` · `storage/cost_repo.py` |
