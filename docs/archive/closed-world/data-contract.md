...
LLM usage by milestone: **M0/M1 require no LLM** (no embeddings, no LLM calls, no live paper APIs). **M2 `grade` (misconception detection), M2 `teach` (grounded explanation), M3 `induce_scaffold`, and M4 semantic retrieval call an LLM/embedding model when a provider is set (`LITNAV_LLM_PROVIDER=openai`/`qwen`), and fall back to deterministic paths when `none`** — so all gates pass fully offline. M2 grade lets the LLM pick the misconception id (validated against candidates); M3 induce lets the LLM **propose the misconception (wrong/correct model) from the real chunks** and label evidence strength (autonomous live induction); `teach` generates a grounded explanation via `complete_text`. Confidence and routing are always rule-computed, never LLM-emitted (see `docs/evaluation.md`).
