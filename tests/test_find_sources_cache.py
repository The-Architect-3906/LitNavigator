...
def test_second_identical_find_is_cache_hit_and_skips_adapters(monkeypatch):
    ...
    monkeypatch.setattr(wikipedia, "search", lambda *a, **k: [])
    monkeypatch.setattr(fulltext, "attach_fulltext", lambda s, *, top_k: None)
    c = sqlite3.connect(":memory:"); init_db(c)
    r1 = find_sources.find(DiscoverInput("goal", k=2), conn=c, session_id="s")
    r2 = find_sources.find(DiscoverInput("goal", k=2), conn=c, session_id="s")
...
def test_different_goal_is_a_miss(monkeypatch):
    ...
    monkeypatch.setattr(fulltext, "attach_fulltext", lambda s, *, top_k: None)
    c = sqlite3.connect(":memory:"); init_db(c)
    find_sources.find(DiscoverInput("goal A", k=2), conn=c, session_id="s")
    find_sources.find(DiscoverInput("goal B", k=2), conn=c, session_id="s")
    assert calls["n"] == 2
